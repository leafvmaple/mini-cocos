...
void framebufferSizeCallback(GLFWwindow* window, int width, int height) {
    auto* view = static_cast<GLViewImpl*>(glfwGetWindowUserPointer(window));
    if (view) {
...
GLViewImpl::~GLViewImpl() { shutdown(); }

bool GLViewImpl::init(int width, int height, const char* title) {
    if (_window) {
        return true;
    }
...
bool GLViewImpl::shouldClose() const { return !_window || glfwWindowShouldClose(_window); }
...
bool GLViewImpl::getMousePosition(float& outX, float& outY) const {
...
double GLViewImpl::getTimeSeconds() const { return glfwGetTime(); }

void GLViewImpl::onFramebufferSize(int width, int height) {
...
void GLViewImpl::onKey(int key, int scanCode, int action, int mods) {
...
void GLViewImpl::onMouseButton(int button, int action, int mods) {
...
void GLViewImpl::onCursorPos(double x, double y) {
    float px = 0.f;
    float py = 0.f;
    toEnginePoint(x, y, px, py);
...
void GLViewImpl::onScroll(double offsetX, double offsetY) {
    double x = 0.0;
    double y = 0.0;
    glfwGetCursorPos(_window, &x, &y);
...
void GLViewImpl::toEnginePoint(double x, double y, float& outX, float& outY) const {
    int windowWidth = 0;
    int windowHeight = 0;
